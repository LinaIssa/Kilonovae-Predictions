...
double pdf(double *I, double *Q, double *U, double *phisc, double *M) {
    ...
    If =  (a+1) * (*I)   +   (a-1) *  (   cos( 2 * (*phisc) ) * (*Q)   +  sin( 2 * (*phisc) ) * (*U)  ); // see equation in paper 2015
...
void rejection(double *I, double *Q, double *U, double *phisc, double *M) {
    ...
    do {
        ...
        /* Sample i1 and M=cos(theta) from an isotropic distribution */ // scaterring defined by 2 angles thetha and phi
        ...
        x = 2 * gsl_rng_uniform(rng) ;  // 2 is the peak 
...
double tau_boundary(double *x, int *photosph_flag, double *ax_source, int *nx, double *dgrid, double *ksc_free, double *ksc_rich, 
    double *kabs_par_free, double *kabs_par_rich, int *tdep, double *temission, double *tcurrent, double *tgrid, double *dens, 
    ...
    tfuture = (*tcurrent);          // updating time and different properties at different timesteps
    ...
    while( inside_grid == 1 && tbound < *tau_max) {
        ...
        eps = (*dgrid) * tfuture / (*tgrid) * frac_eps ;    // size of the cell + factor of expansion
        ...
        if (inside_grid == 1) { //calcul of the optical depth
            ...
            // Increment path
            *dbound += eps ;    //distance btw the location and the boundary 
...
double tau_cont(double *x, double *ksc_free, double *ksc_rich, double *kabs_par_free, double *kabs_par_rich, 
...
double s_cont(double *x, double *ksc_free, double *ksc_rich, double *kabs_par_free, double *kabs_par_rich, 
...
void source_radio(int *nx, double *dgrid, double *xgrid, double *ygrid, double *zgrid, 
    ...
    u = 2. * gsl_rng_uniform(rng) - 1.;     // u = cos(thetha) coodonnées polaires
    v = 2. * PI * gsl_rng_uniform(rng);
...
///****************************************************************************
double sample_planck(double T0, double alphaT, double tcurrent, double nu_min_r, double nu_max_r)
{
  double zrand,zrand2;
  double nu,nu_peak;
  double B_peak;
...
///****************************************************************************
double sample_emissivity(double *T0, double *alphaT, double *tcurrent, double *tgrid, int *nx, double *dgrid, double *dens, double *Ye, double *Ye_crit, 
    double *x, double *ksc_free, double *ksc_rich, double *kabs_par_free, double *kabs_par_rich, double *emiss, double *nu_cmf) {
  ...
  // nu from Bnu x kappa_nu (emissivity)
      ...
      snu_peak = 0;     // emissivity see paper 2019
      
      // Density and Ye for given cell
      read_cell(x,tcurrent,tgrid,nx,dgrid,dens,Ye,&dens_cell,&Ye_cell) ;  // tells the cell
      ...
      for (inu=0; inu<Nnu; inu++) {
        ...
        else knu = abs_coefficient(&nu,kabs_par_free,tcurrent) ;
      ...
      // Do the frequency sampling   // rejection technique
      endloop = 0;
      while (endloop == 0)
      {
        zrand2 = gsl_rng_uniform(rng);
