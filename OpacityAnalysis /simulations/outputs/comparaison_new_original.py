...
data_fold = Path('outputs_nph5/')
data_foldBand =  Path('outputs_file/')
file_ref  = data_foldBand / f'nph{Nph}_mejdyn{MejDyn}_mejwind{MejWind}_phi{Phi}_spec_ref.txt'
#file_sin  = data_fold / f'nph{Nph}_mejdyn{MejDyn}_mejwind{MejWind}_phi{Phi}_spec_sin.txt'

file_sin   = data_fold /f'nph1.0e+05_kappaLF{kappaLF}_gammaLF{gammaLF}_kappaLR{kappaLR}_gammaLR{gammaLR}_spec.txt'


data_time = []
data_magn = []
for f in ['g', 'r', 'i', 'h']:
    file_data_f = data_foldBand / f'data_{f}.txt'
...
for (band,f, i) in zip(filter, [0,1,2,3], range(l*4,l*4+4,1)):
    dataY_ref = list(m_ref[f][0])
    dataY_sin = list(m_sin[f][0])
    numPlot = 221 + i
...
Ax.set_title(r'Light curves drawn for $\theta_{\rm{obs}} = 45 \degree$', x = 0, y =2.3, fontsize = 10)


fig.show()
output = f'lc_nph{Nph}_kappaLF{kappaLF}_gammaLF{gammaLF}_kappaLR{kappaLR}_gammaLR{gammaLR}.pdf'
