...
double tau_boundary(double *x, int *photosph_flag, double *ax_source, int *nx, double *dgrid, double *ksc_free, double *ksc_rich, double *ksc_int, 
    ...
    // Break calculation if tfuture>tmax (optically thin ejecta, no need to increment opacity)
    tmax = 100 * DAY ;    
    ...
    while( inside_grid == 1 && tbound < *tau_max && tfuture < tmax) {
        ...
        if (inside_grid == 1) {
            ...
            if (Ye_cell<(*Ye_crit)) {
                ...
                kabstmp = abs_coefficient_2(&nu_cmf,kabs_par_rich,&tfuture) ;
                ksctmp = (*ksc_rich) * pow(tfuture/(*tgrid),kabs_par_rich[3]) ; 
            }

            else if (Ye_cell>(*Ye_crit)) {

                kabstmp = abs_coefficient_2(&nu_cmf,kabs_par_free,&tfuture) ;
                ksctmp = (*ksc_free) * pow(tfuture/(*tgrid),kabs_par_free[3]) ; 
            }

            else {

                kabstmp = abs_coefficient_2(&nu_cmf,kabs_par_int,&tfuture) ;
                ksctmp = (*ksc_int) * pow(tfuture/(*tgrid),kabs_par_int[3])  ; 
            }
...
double s_cont(double *x, double *ksc_free, double *ksc_rich, double *ksc_int, double *kabs_par_free, double *kabs_par_rich, double *kabs_par_int, 
    ...
    while( tautmp < (*tau_ev) - (*tau_lines)  && tfuture < tmax ) {
...
double abs_coefficient_2(double *nu, double *kabs_par, double *tcurrent) {
    ...
    wave = CLIGHT / (*nu) * pow(10., 8)  ;// in Angström 

    // Calculate the slope time dependency 

    alphat = kabs_par[2] * pow(*tcurrent/ (1.5 * DAY), kabs_par[3]) ;

    // Calculate logkabs for different wavelength ranges

    if (wave < kabs_par[0]) logkabs = log10( kabs_par[1]) ;
    else logkabs = log10( kabs_par[1]) - alphat * (log10(wave) - log10(kabs_par[0]));   
    kabs = pow(10., logkabs) ;
...
double temperature(double *tcurrent, double *tgrid, double *dgrid, int *nx, double *x, double *Ye, double *dens, double *eps_data, double *rate_data, double *time_data, int Nstep_data){
    ...
    for (i=0; i<index; i++){
        // Last factor to account for adiabatic energy loss
...
double sample_emissivity(double T, double *tcurrent, double *tgrid, int *nx, double *dgrid, double *dens, double *Ye, double *Ye_crit, 
  ...
  // nu from Bnu x kappa_nu (emissivity)
      ...
      for (inu=0; inu<Nnu; inu++) {
        ...
        // Opacity
        if (Ye_cell<(*Ye_crit)) knu = abs_coefficient_2(&nu,kabs_par_rich,tcurrent) ;
        else if (Ye_cell>(*Ye_crit)) knu = abs_coefficient_2(&nu,kabs_par_free,tcurrent) ;
        else knu = abs_coefficient_2(&nu,kabs_par_int,tcurrent) ;
