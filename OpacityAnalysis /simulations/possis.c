...
int main() {
    ...
    double Nph,Nph_dt, kappa_LF, gamma_LF, kappa_LR, gamma_LR, Mej_dyn, Mej_wind, open_angle ;
    int lf_contr;
    ...
    // Start simulation
    for (isim=0;isim<Nsim;isim++) {
    	kappa_LF = sim_par[isim][0] ;
    	gamma_LF = sim_par[isim][1] ;
    	kappa_LR = sim_par[isim][2] ;
    	gamma_LR = sim_par[isim][3] ;
    	printf("%f, %f, %f, %f \n", kappa_LF, gamma_LF, kappa_LR, gamma_LR);

        // Check if simulation is present already
        FILE *fq, *finfo, *fgrid, *fenergy;//, *fopac ;
        char filespec[128],fileinfo[128],filegrid[128], fileenergy[128]; //fileopac[128] ;
        char str[2];
        int flag_sim=1;

        if (lf_contr == 1) {
         
            snprintf(filespec,sizeof filespec,"outputs/nph%.1e_kappaLF%.0f_gammaLF%.2f_kappaLR%.0f_gammaLR%.2f_spec.txt",
                Nph, kappa_LF, gamma_LF, kappa_LR, gamma_LR) ;
        	snprintf(fileinfo,sizeof fileinfo,"outputs/nph%.1e_kappaLF%.0f_gammaLF%.2f_kappaLR%.0f_gammaLR%.2f_info.txt",
            	Nph,kappa_LF, gamma_LF, kappa_LR, gamma_LR) ;
        	//snprintf(fileopac, sizeof fileopac, "outputs/opacity_profile_nph%.1e.txt", Nph) ; 
        }
        ...
        if (flag_sim == 1) {
            ...
            double ksc_free,ksc_rich,ksc_int,kabs,ksc,alb=0;
            double n_obs[3];
            ...
            int inside;
            double Temp, mout,eps_tot,factor,dMpc,Ye_crit,dens_cell,Ye_cell;
            double *msum, *xgrid, *ygrid, *zgrid, *dens, *Ye;
            double tcurrent, tgrid, mej_grid_dyn,mej_grid_wind, tarrive_vpkt, ti, tf, step_t;
            int tdep,Ntime,itime;
            double kabs_par_free[5], kabs_par_rich[5],kabs_par_int[5];
            ...
            snprintf(filegrid,sizeof filegrid,"models/grid_input_phi%.0f.txt",open_angle);
            long seed;
            seed = 1 ;//time(NULL);
            ...
            // --- lanthanide-free
            kabs_par_free[0] = 2818 ;    //  lambda_0 in Angström
            kabs_par_free[1] = kappa_LF   ;    //  kapp_0 at lambda_0 in cm2/g
            kabs_par_free[2] = 10.8 ;    //  slope for wavelength dependency
            if (tdep == 1) kabs_par_free[3] = gamma_LF; // slope for time dependency (gamma)
            else kabs_par_free[3] = 0 ; //no time-dependence            
 

            // --- lanthanide-rich
            kabs_par_rich[0] = 4467 ;
            kabs_par_rich[1] = kappa_LR ; // in cm2/g
            kabs_par_rich[2] = 8.86;
            if (tdep == 1) kabs_par_rich[3] = gamma_LR ; // slope for time dependency (gamma)
            else kabs_par_rich[3] = 0 ; //o time-dependence

            // --- lanthanide-intermediate

            kabs_par_int[0] = (kabs_par_rich[0] + kabs_par_free[0])/2 ;
            kabs_par_int[1] = (kabs_par_rich[1] + kabs_par_free[1])/2 ; // in cm2/g
            //kabs_par_int[2] = (kabs_par_rich[2] + kabs_par_free[2])/2;
            kabs_par_int[2] = (kabs_par_rich[2] + kabs_par_free[2])/2 ;
            if (tdep == 1) kabs_par_int[3] = (kabs_par_rich[3] + kabs_par_free[3])/2; // slope for time dependency (gamma)
            else kabs_par_int[3] = 0 ; //o time-dependence
            ...
            for (i=0;i<Nobs;i++) {
                ...
                //else costobs = 1  ;
                else costobs = cos(17 * PI/180) ;  
                tobs[i]   = acos(costobs);  // degrees
                phiobs[i] = tobs[i]*PI/180; // radian  
            ...
            for(i=0;i<ngrid_tot;i++){
                ...
                if ( sqrt(pow(xgrid[i],2.)+pow(ygrid[i],2.)+pow(zgrid[i],2.)) / tgrid > 0.08 * CLIGHT ) mej_grid_dyn += pow(dgrid,3.) * dens[i] ;
                else mej_grid_wind += pow(dgrid,3.) * dens[i] ;

            }
            
            //printf("%f %f \n", mej_grid_dyn/MSUN, mej_grid_wind/MSUN) ;
            
            // Scale to required mass
            for(i=0;i<ngrid_tot;i++) {

                if ( sqrt(pow(xgrid[i],2.)+pow(ygrid[i],2.)+pow(zgrid[i],2.)) / tgrid > 0.08 * CLIGHT ) 
                    dens[i] = dens[i] * (Mej_dyn * MSUN/mej_grid_dyn) ;
                else dens[i] = dens[i] * (Mej_wind* MSUN/mej_grid_wind) ;
            }
            ...
            for (itime=0;itime<Ntime;itime++) {
                ...
                vphot = photosphere(&nx,&dgrid,&tgrid,&t,dens,&ksc_free,&kabs_par_free[3]) ;
                ...
                for(i=0;i<Nph_dt;i++) {
                    ...
                    nu_rpkt_cmf = sample_emissivity(Temp,&tcurrent,&tgrid,&nx,&dgrid,dens,Ye,&Ye_crit,
                        x,&ksc_free,&ksc_rich,&ksc_int,kabs_par_free,kabs_par_rich,kabs_par_int,&eps_emisstmp,&nu_rpkt_cmf);
                    ...
                    //    if (Ye_cell<Ye_crit) {
                    //        kabs = abs_coefficient_2(&nu_rpkt_cmf,kabs_par_rich,&tcurrent) ;
                    //  	    fprintf(fopac, "%f \t  %f \n", tcurrent/DAY, kabs);
                    ...
                    //        kabs = abs_coefficient_2(&nu_rpkt_cmf,kabs_par_free,&tcurrent) ;
                    //    	fprintf(fopac, "%f \t %f \n", tcurrent/DAY, kabs);
                    //    }

                    //    else {
                                
                    //         kabs = abs_coefficient_2(&nu_rpkt_cmf,kabs_par_int,&tcurrent) ;
                    //         fprintf(fopac, "%f \t  %f \n", tcurrent/DAY, kabs);
                    //    }
                    ...
                    for (l=0;l<Nobs;l++) {
                        ...
                        eps_vpkt_rf = eps_rpkt_cmf * nu_vpkt_rf / nu_rpkt_cmf ;
                        ...
                        if (fabs(tarrive_vpkt) < tf) {
                            ...
                            tesc = tau_boundary(x,&photosph_flag,ax_photosph,&nx,&dgrid,&ksc_free,&ksc_rich,&ksc_int,
                                kabs_par_free,kabs_par_rich,kabs_par_int,&tdep,&t,&tcurrent,&tgrid,dens,Ye,&Ye_crit,n_obs,&nmax_eps,&tau_vpkt_max,&dbound,&nu_vpkt_rf);
                    ...
                    eps_rpkt_rf = eps_rpkt_cmf  * nu_rpkt_rf / nu_rpkt_cmf ;
                    ...
                    while ( tcurrent < tf + 0.5 * nx * dgrid * tf / tgrid / CLIGHT ) {  // Kill real packet if tcurrent larger than tf + N*day (tarrive != tcurrent, see below)
                        ...
                        for (ind=0;ind<NLINE;ind++) {
                            ...
                            if (color[ind]==1) {
                                ...
                                tau_c[ind] = tau_cont(x,&ksc_free,&ksc_rich,&ksc_int,kabs_par_free,kabs_par_rich,kabs_par_int,
                        ...
                        while (ind < NLINE) {
                            ...
                            if (tau_c[ind] > tau_ev) {
                                ...
                                if (ind==0) {
                                    ...
                                    tau_sob_cum = 0 ;
                                    s = s_cont(x,&ksc_free,&ksc_rich,&ksc_int,kabs_par_free,kabs_par_rich,kabs_par_int,
                        ...
                        if ( flag_cont == 1 ) {
                            ...
                            eps_rpkt_cmf = eps_rpkt_rf * nu_rpkt_cmf / nu_rpkt_rf ;
                            ...
                            if (Ye_cell<Ye_crit) {
                                ...
                                kabs = abs_coefficient_2(&nu_rpkt_cmf,kabs_par_rich,&tcurrent) ;
                                ksc = ksc_rich * pow((tcurrent/tgrid),kabs_par_rich[3]) ; 
                        	    //fprintf(fopac, "%f \t  %f \n", kabs, ksc);
                                ...
                                kabs = abs_coefficient_2(&nu_rpkt_cmf,kabs_par_free,&tcurrent) ;
                                ksc = ksc_free * pow((tcurrent/tgrid),kabs_par_free[3]) ; 
                        	    //fprintf(fopac, "%f \t %f \n", kabs, ksc);
                            }
                                ...
                                kabs = abs_coefficient_2(&nu_rpkt_cmf,kabs_par_int,&tcurrent) ;
                                ksc = ksc_int * pow((tcurrent/tgrid),kabs_par_int[3]) ; 
                                //fprintf(fopac, "%f \t  %f \n", kabs, ksc);
                            }
                            ...
                            if(g>alb) {
                                ...
                                nu_rpkt_cmf  = sample_emissivity(Temp,&tcurrent,&tgrid,&nx,&dgrid,dens,Ye,&Ye_crit,
                                    x,&ksc_free,&ksc_rich,&ksc_int,kabs_par_free,kabs_par_rich,kabs_par_int,&eps_emiss,&nu_rpkt_cmf);
                                ...
                                for (l=0;l<Nobs;l++) {
                                    ...
                                    if (fabs(tarrive_vpkt) < tf) {
                                        ...
                                        tesc = tau_boundary(x,&photosph_flag,ax_photosph,&nx,&dgrid,&ksc_free,&ksc_rich,&ksc_int,kabs_par_free,
                                            kabs_par_rich,kabs_par_int,&tdep,&t,&tcurrent,&tgrid,dens,Ye,&Ye_crit,n_obs,&nmax_eps,&tau_vpkt_max,&dbound,&nu_vpkt_rf);
