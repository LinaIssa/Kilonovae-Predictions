...
int main() {
    ...
    for (isim=0;isim<Nsim;isim++) {
        ...
        if (lf_contr == 1) {
            ...
            snprintf(filespec,sizeof filespec,"outputs/nph%.1e_mejdyn%.3f_mejwind%.3f_phi%.0f_spec.txt",
                Nph,sim_par[isim][0],sim_par[isim][1],sim_par[isim][2]) ;
            snprintf(fileinfo,sizeof fileinfo,"outputs/nph%.1e_mejdyn%.3f_mejwind%.3f_phi%.0f_info.txt",
                Nph,sim_par[isim][0],sim_par[isim][1],sim_par[isim][2]) ;
        ...
        if (flag_sim == 1) {
            ...
            snprintf(filegrid,sizeof filegrid,"models/grid_input_phi%.0f.txt",sim_par[isim][2]);
            long seed;
            seed = 1 ;//time(NULL);
            ...
            kabs_par_int[0] = 0 ; // lambda1 in cm (constant with time)
            kabs_par_int[1] = 1e2 ; // kappa for lambda < lambda1 (constant with time)
            kabs_par_int[2] = 1e4 * 1e-8 ; // lambda2 in cm (constant with time)
            kabs_par_int[3] = 0.1 ; // kappa for lambda > lambda2 (changing with time)
            if (tdep == 1) kabs_par_int[4] = 1.0 ; // index of time-dependence opacity
            else kabs_par_int[4] = 0 ; //o time-dependence
            ...
            for(i=0;i<ngrid_tot;i++) {
                ...
                if ( sqrt(pow(xgrid[i],2.)+pow(ygrid[i],2.)+pow(zgrid[i],2.)) / tgrid > 0.08 * CLIGHT ) 
                    dens[i] = dens[i] * (sim_par[isim][0] * MSUN/mej_grid_dyn) ;
                else dens[i] = dens[i] * (sim_par[isim][1] * MSUN/mej_grid_wind) ;
            }
                    ...
                    if (lf_contr == 1) read_cell(x,&tcurrent,&tgrid,&nx,&dgrid,dens,Ye,&dens_cell,&Ye_cell);
                        ...
                        if (Ye_cell<Ye_crit) {
                            ...
                            fprintf(fopac, "%f \t %f \n", tcurrent/DAY, kabs);
                             ...
                             kabs = abs_coefficient(&nu_rpkt_cmf,kabs_par_int,&tcurrent) ;
                             fprintf(fopac, "%f \t  %f \n", tcurrent/DAY, kabs);
                        }
                    ...
                    while ( tcurrent < tf + 0.5 * nx * dgrid * tf / tgrid / CLIGHT ) {  // Kill real packet if tcurrent larger than tf + N*day (tarrive != tcurrent, see below)
                        ...
                        if ( flag_cont == 1 ) {
                            ...
                            if (Ye_cell<Ye_crit) {
                                ...
                                kabs = abs_coefficient(&nu_rpkt_cmf,kabs_par_int,&tcurrent) ;
                                ksc = ksc_int * pow((tgrid/tcurrent),kabs_par_int[4]) ; 
                            }
