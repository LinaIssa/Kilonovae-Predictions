...
int main() {
    ...
    for (isim=0;isim<Nsim;isim++) {
        ...
        FILE *fq, *finfo, *fgrid, *fenergy, *ftemp ;
        char filespec[128],fileinfo[128],filegrid[128], fileenergy[128], filetemp[128] ;
        char str[2];
        int flag_sim=1;
        ...
        snprintf(fileinfo,sizeof fileinfo,"outputs/nph%.1e_mejdyn%.3f_mejwind%.3f_phi%.0f_info.txt",
             Nph,sim_par[isim][0],sim_par[isim][1],sim_par[isim][2]) ;
        snprintf(filetemp,sizeof filetemp, "outputs/temp_profile_vph_004.txt"); 
        ...
        if (flag_sim == 1) {
            ...
            snprintf(filegrid,sizeof filegrid,"models/grid_input_phi%.0f_sin_dyn.txt",sim_par[isim][2]);
            ...
            for (itime=0;itime<Ntime;itime++) {
                ...
                // Scaling factor to transform I stokes parameter to Flux density (1/4pi already included in prob)
                    ...
                    while ( tcurrent < tf + 0.5 * nx * dgrid * tf / tgrid / CLIGHT ) {  // Kill real packet if tcurrent larger than tf + N*day (tarrive != tcurrent, see below)
                        ...
                        if ( flag_cont == 1 ) {
                            ...
                            if(g>alb) {
                                ...
                                Temp = temperature(&tcurrent,&tgrid,&dgrid,&nx,x,Ye,dens,eps_data,rate_data,time_data,Nstep_data) ;
                                //Temp = 5000 * pow( tcurrent/DAY,-0.4 );
